...
public class Search {
    ...
    public static SendMessage filmInlineKeyboardInit(long chat_id, int pageNumber, String inMessage) {
        setFullNameList();
        setNameYearList(getFullNameList());
        setPartNameList(getNameYearList());
        setCallbackMap();
        setInlineKeyboardRows(pageNumber, inMessage);
...
//        mongoClient.close();
        return SendMessage.builder()
                ...
                .text("Кино не умрет, пока в кинотеатрах будет темно.")
                .build();
    }

    private static synchronized void setInlineKeyboardRows(int pageNumber, String inMessage) {
        inlineKeyboardRows.clear();
        List<String> fileNameList = getNameYearList();
        //кнопка с колбэком на кнопку фильма
        for (String fileNameYear : fileNameList) {
            if (fileNameYear.toLowerCase().contains(inMessage.toLowerCase())) {
                System.out.println(fileNameYear);
        ...
        if (inlineKeyboardRows.size() == 10 && pageNumber < 1) {
            InlineKeyboardRow keyboardRow = new InlineKeyboardRow();
        ...
        if (inlineKeyboardRows.size() != 11 && pageNumber >= 1) {
            InlineKeyboardRow keyboardRow = new InlineKeyboardRow();
            keyboardRow.add(InlineKeyboardButton.builder().text("◀ Назад").callbackData(getCallback(pageNumber - 1)).build());
            inlineKeyboardRows.add(keyboardRow);
        }
